...
function isSuperKiroInstalled(pipCmd) {
  const result = run(pipCmd, ["show", "SuperKiro"]);
  return result.status === 0;
}

function isSuperKiroInstalledPipx() {
  const result = run("pipx", ["list"]);
  if (result.status === 0 && result.stdout) {
    return result.stdout.toString().includes("SuperKiro");
  }
  return false;
}
...
module.exports = { run, detectPython, detectPip, detectPipx, isSuperKiroInstalled, isSuperKiroInstalledPipx, checkPythonEnvironment };
