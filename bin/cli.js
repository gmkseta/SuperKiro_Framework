...
if (!noUpdateCheck && !isQuiet) {
  ...
  checkAndNotify({
    ...
    if (updated) {
      console.log("\n🔄 SuperKiro was updated. Please restart to use the new version.");
      process.exit(0);
    }
  }).catch(() => {
...
// Forward everything to Python SuperKiro
const result = spawnSync(pythonCmd, ["-m", "SuperKiro", ...args], { stdio: "inherit", shell: true });
