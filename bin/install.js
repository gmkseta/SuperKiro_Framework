#!/usr/bin/env node
const { run, detectPython, detectPip, detectPipx, isSuperKiroInstalled, isSuperKiroInstalledPipx, checkPythonEnvironment } = require("./checkEnv");
...
if (isExternallyManaged) {
  ...
  if (pipxCmd) {
    ...
    console.log("     pip install --user SuperKiro  # Recommended");
    console.log("     pip install --break-system-packages SuperKiro  # Force (use with caution)");
    ...
    if (pipCmd) {
      ...
      isInstalled = isSuperKiroInstalled(pipCmd);
    } else {
...
if (!isInstalled) {
  console.log("📦 Installing SuperKiro from PyPI...");
  
  let result;
  switch(installMethod) {
    case "pipx":
      result = run("pipx", ["install", "SuperKiro"], { stdio: "inherit" });
      break;
    case "pip-user":
      result = run(detectPip(), ["install", "--user", "SuperKiro"], { stdio: "inherit" });
      break;
    case "pip":
      result = run(detectPip(), ["install", "SuperKiro"], { stdio: "inherit" });
      break;
  }
  ...
  if (result.status !== 0) {
    ...
    if (installMethod === "pip" && isExternallyManaged) {
      ...
      console.error("   Try: pipx install SuperKiro");
      console.error("   Or:  pip install --user SuperKiro");
    }
    process.exit(1);
  }
  console.log("✅ SuperKiro installed successfully!");
  
  // For pipx installations, ensure it's in PATH
  if (installMethod === "pipx") {
    console.log("\n📌 Note: If 'SuperKiro' command is not found, run:");
    console.log("   pipx ensurepath");
    console.log("   Then restart your terminal or run: source ~/.bashrc");
  }
} else {
  console.log("✅ SuperKiro already installed.");
}

// Try to run SuperKiro install
console.log("\n🚀 Running SuperKiro installation...");
const installResult = run("SuperKiro", ["install"], { stdio: "inherit" });

if (installResult.status !== 0) {
  console.log("\n⚠️  Could not run 'SuperKiro install' automatically.");
  console.log("   Please run it manually after ensuring SuperKiro is in your PATH:");
  console.log("   SuperKiro install");
