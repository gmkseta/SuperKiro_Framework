"""
SuperKiro Uninstall Operation Module
...
from ...utils.environment import get_superkiro_environment_variables, cleanup_environment_variables
from ...utils.logger import get_logger
from ... import DEFAULT_INSTALL_DIR, PROJECT_ROOT
from . import OperationBase


def verify_superkiro_file(file_path: Path, component: str) -> bool:
    """
    Verify this is a SuperKiro-managed file before removal (backward-compatible)
    ...
    """
    try:
        # Known SuperKiro/SuperClaude file patterns by component (backward-compatible)
        superkiro_patterns = {
            'core': [
        ...
        if component in superkiro_patterns:
            filename = file_path.name
            return filename in superkiro_patterns[component]
...
def verify_directory_safety(directory: Path, component: str) -> bool:
    ...
    Returns:
        True if safe to remove (only if empty or only contains known framework files)
    """
    try:
        ...
        # Check if all contents are known framework files for this component
        for item in contents:
            if item.is_file():
                if not verify_superkiro_file(item, component):
                    return False
            elif item.is_dir():
                # Don't remove directories that contain non-framework subdirectories
                return False
...
Examples:
  ...
  SuperKiro uninstall --keep-backups     # Keep backup files
    ...
    # Project steering maintenance (non-global uninstall): remove only SuperKiro steering templates
    parser.add_argument(
        "--prune-steering",
        action="store_true",
    ...
    parser.add_argument(
        "--cleanup-env",
        action="store_true",
        help="Remove SuperKiro-related environment variables"
    )
...
def display_environment_info() -> Dict[str, str]:
    """Display SuperKiro-related environment variables and return them"""
    env_vars = get_superkiro_environment_variables()
    
    if env_vars:
        print(f"\n{Colors.CYAN}{Colors.BRIGHT}Environment Variables{Colors.RESET}")
        print("=" * 50)
        print(f"{Colors.BLUE}SuperKiro-related API key environment variables found:{Colors.RESET}")
        ...
        print(f"\n{Colors.GREEN}No SuperKiro-related environment variables found{Colors.RESET}")
...
def display_uninstall_info(info: Dict[str, Any]) -> None:
    ...
    if not info["exists"]:
        print(f"{Colors.YELLOW}No SuperKiro installation found{Colors.RESET}")
        return
...
def interactive_component_selection(installed_components: Dict[str, str], env_vars: Dict[str, str]) -> Optional[tuple]:
    ...
    print(f"\n{Colors.CYAN}{Colors.BRIGHT}SuperKiro Uninstall Options{Colors.RESET}")
    print("=" * 60)
    
    # Main uninstall type selection
    main_options = [
        "Complete Uninstall (remove all SuperKiro components)",
...
def _ask_complete_uninstall_options(env_vars: Dict[str, str]) -> Dict[str, bool]:
    ...
    print("This will remove ALL SuperKiro components.")
...
def _custom_component_selection(installed_components: Dict[str, str], env_vars: Dict[str, str]) -> Optional[tuple]:
    ...
    print("Select which SuperKiro components to remove:")
...
def _ask_mcp_cleanup_options(env_vars: Dict[str, str]) -> Dict[str, bool]:
    ...
    remove_configs = confirm("Remove MCP server configurations from .kiro.json?", default=True)
...
def display_preservation_info() -> None:
    ...
    print(f"{Colors.GREEN}✓ User's custom agents (not SuperKiro agents){Colors.RESET}")
    ...
    print(f"{Colors.GREEN}✓ Kiro settings and other tools' configurations{Colors.RESET}")
...
def display_component_details(component: str, info: Dict[str, Any]) -> Dict[str, Any]:
    ...
    component_paths = {
        ...
        'commands': {
            ...
            'description': 'SuperKiro commands in ~/.claude/commands/sc/'
        },
        'agents': {
            'files': 'agents/*.md',
        ...
        'modes': {
            'files': 'MODE_*.md',
            'description': 'SuperKiro operational modes'
        }
    }
...
def display_uninstall_plan(components: List[str], args: argparse.Namespace, info: Dict[str, Any], env_vars: Dict[str, str]) -> None:
    ...
    if args.complete:
        print(f"\n{Colors.RED}⚠️  WARNING: Complete uninstall will remove all SuperKiro files{Colors.RESET}")
...
def run(args: argparse.Namespace) -> int:
    ...
    # Lightweight project steering prune path (runs before global uninstall validations)
    if getattr(args, "prune_steering", False):
        try:
    ...
    try:
        ...
        env_vars = display_environment_info() if not args.quiet else get_superkiro_environment_variables()
        
        # Check if SuperKiro is installed
        if not info["exists"]:
            logger.warning(f"No SuperKiro installation found in {args.install_dir}")
            return 0
        ...
        if not args.no_confirm and not args.yes:
            if args.complete:
                warning_msg = "This will completely remove SuperKiro. Continue?"
            else:
        ...
        if success:
            if not args.quiet:
                display_success("SuperKiro uninstall completed successfully!")
                
                if not args.dry_run:
                    print(f"\n{Colors.CYAN}Uninstall complete:{Colors.RESET}")
                    print(f"SuperKiro has been removed from {args.install_dir}")
                    if not args.complete:
                        print(f"You can reinstall anytime using 'SuperKiro install'")
